...
class JobDescription(models.Model):
    ...
    Used for job matching, cover letter generation, and skill gap analysis.
    Can extract required and preferred skills from job posting text.
    """
    
    # User who added this job description
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, 
    ...
    title = models.CharField(
        max_length=255,
        help_text="Job title or position name (e.g., 'Senior Python Developer')"
    )
    ...
    # Skills parsed from job description
    required_skills = models.JSONField(
        default=list,  # Empty list by default
        blank=True,
        help_text="Mandatory skills required for the position"
    )
    preferred_skills = models.JSONField(
        default=list, 
        blank=True,
        help_text="Nice-to-have skills that are not mandatory"
    )
    ...
    def __str__(self):
        """Return readable representation with job title and company"""
...
class CoverLetter(models.Model):
    """Model for storing AI-generated, job-specific cover letters
    
    Links a resume with a job description to generate personalized
    cover letters using Gemini AI.
    """
    
    # Resume being used for this application
    resume = models.ForeignKey(
        Resume, 
        on_delete=models.CASCADE,  # Delete cover letter if resume is deleted
        help_text="Resume used to generate this cover letter"
    )
    
    # Job being applied to
    job_description = models.ForeignKey(
        JobDescription, 
        on_delete=models.CASCADE,  # Delete cover letter if job is deleted
        help_text="Job description for this application"
    )
    
    # AI-generated cover letter content
    generated_text = models.TextField(
        help_text="Complete cover letter text generated by AI"
    )
    ...
    def __str__(self):
        """Return readable representation with job title"""
